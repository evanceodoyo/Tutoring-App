...
class LoginViewTests(TestCase):
    ...
    def test_login_view_displays_error_message_if_invalid_credentials(self):
        ...
        self.assertEqual(
            str(messages[0]), 'Invalid email or password!'
        )
        self.assertEqual(response.status_code, 200)

    def test_login_view_logs_in_user_and_redirects_to_next_if_valid_credentials(self):
        response = self.client.post(self.url, {'email': 'testuser@mail.com', 'password': 'secret', 'next': reverse('courses')})
        self.assertEqual(response.status_code, 302)
    ...
    def test_login_view_logs_in_user_and_redirects_to_home_if_valid_credentials(self):
        response = self.client.post(reverse('login'), {'email': 'testuser@mail.com', 'password': 'secret'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('home'))
        self.assertTrue(self.client.session.get('_auth_user_id'))
...
class ProfileViewTests(TestCase):
    ...
    def test_profile_view_does_not_display_courses_enrolled_by_other_users(self):
        other_user = User.objects.create_user(name="Other Test User", username='testuser2', email='testuser2@mail.com', password='secret')
...
class ProfileUpdateViewTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(name="Test User", username='testuser', email='testuser@mail.com', password='secret', address='Test Address', phone='0711223344')
    ...
    def test_profile_update_view_POST_redirects_to_profile_if_name_provided(self):
        ...
        response = self.client.post(self.url, {'name': 'Test User Changed', 'phone': '0712345678', 'address': 'Test Address Changed', 'bio': 'Test Bio'})
        self.assertEqual(response.status_code, 302)
    ...
    def test_profile_update_view_displays_error_message_if_name_not_provided(self):
        self.client.login(email='testuser@mail.com', password='secret')
        response = self.client.post(self.url, {'phone': '0712345678', 'address': 'Test Address Changed', 'bio': 'Test Bio'})
...
class PasswordChangeViewTests(TestCase):
    ...
    def test_password_change_view_displays_error_message_if_passwords_dont_match(self):
        self.client.login(email='testuser@mail.com', password='secret')
        response = self.client.post(self.url, {'current_password': 'secret', 'new_password': 'newpassword', 'new_password2': 'newpassword2'})
    ...
    def test_password_change_view_displays_error_message_if_current_password_is_wrong(self):
        self.client.login(email='testuser@mail.com', password='secret')
        response = self.client.post(self.url, {'current_password': 'wrongpassword', 'new_password': 'newpassword', 'new_password2': 'newpassword'})
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('profile'))
...
class PasswordResetViewTests(TestCase):
    ...
    def test_password_reset_view_displays_error_message_if_user_does_not_exist(self):
        ...
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), 'Sorry, user with the provided email does not exist.')
...
class NewsLetterSubscriptionViewTests(TestCase):
    ...
    def test_newsletter_subscription_view_subscribes_user_if_not_already_subscribed(self):
        response = self.client.post(self.url, {'email': 'testsubscriber@example.com'}, HTTP_REFERER=self.referrer_url)
        self.assertEqual(response.status_code, 302)
        ...
        self.assertTrue(NewsLetterSubscriber.objects.filter(email='testsubscriber@example.com').exists())

    def test_newsletter_subscription_view_displays_error_message_if_already_subscribed(self):
        NewsLetterSubscriber.objects.create(email='testsubscriber@example.com')
        response = self.client.post(self.url, {'email': 'testsubscriber@example.com'}, HTTP_REFERER=self.referrer_url)
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, self.referrer_url)
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(str(messages[0]), "You've already subscribed to our newsletter.")